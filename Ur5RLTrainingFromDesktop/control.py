...
from urx.ursecmon import ParserUtils
...
def get_data(s):
    ...
    parse = ParserUtils()
    dict_data = parse.parse(data)
    print(dict_data.keys())
    if 'RobotModeData' in dict_data.keys():
        for item, value in dict_data['RobotModeData'].items():
            print(item, ':', value, '\n')
    # if 'JointData' in dict_data.keys():
    #     for item, value in dict_data['JointData'].items():
    #         print(item, ':', value, '\n')
    # if 'CartesianInfo' in dict_data.keys():
    #     for item, value in dict_data['CartesianInfo'].items():
    #         print(item, ':', value, '\n')
    ...
    if data:
        ...
        if packtype == 16:
            ...
            if robotMessageType == 7:
                robotMessageCode = (struct.unpack('!i', data[15:19]))[0]
                # print('code:', robotMessageCode)
                robotMessageArgument = (struct.unpack('!i', data[19:23]))[0]
                # print('argument:', robotMessageArgument)
                # robotMessageType = (struct.unpack('!b', data[19+i:20+i]))[0] 
                # if robotMessageType == 7:
                #     pass
                #     robotMessageCode = struct.unpack('!b', data[15:19])[0] 

                #     print(robotMessageType)
                #     print(robotMessageCode)
                # else:
                #     pass
